...
// ------------------------------------------------------
// -------------Merchant-scoped variants------------------
// ------------------------------------------------------

const addPromoCodeByMerchantController = async (req, res, next) => {
  // Validate request body
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    let formattedErrors = {};
  ...
  try {
    ...
    // Force the code to the authenticated merchant (ignore any client-supplied value)
    await PromoCode.create({
      promoCode: promoCode.toUpperCase(),
      promoType,
      discount,
...
const getAllPromoCodesByMerchantController = async (req, res, next) => {
  try {
    const promoCodes = await PromoCode.find({ merchantId: req.merchantId })
...
const getSinglePromoCodeByMerchantController = async (req, res, next) => {
  try {
    const { promoCodeId } = req.params;
...
const editPromoCodeStatusByMerchantController = async (req, res, next) => {
  try {
    const { promoCodeId } = req.params;
    const promoCode = await PromoCode.findOne({
      _id: promoCodeId,
...
const deletePromoCodeByMerchantController = async (req, res, next) => {
  try {
    const { promoCodeId } = req.params;
    ...
    if (!deletedPromoCode) return next(appError("Promo code not found", 404));
