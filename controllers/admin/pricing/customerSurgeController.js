...
const addCustomerSurgeController = async (req, res, next) => {
  ...
  try {
    ...
    let newRule = await CustomerSurge.create({
      ruleName,
      baseFare,
      baseDistance,
      waitingFare,
...
const editCustomerSurgeController = async (req, res, next) => {
  ...
  try {
    ...
    let updatedCustomerSurge = await CustomerSurge.findByIdAndUpdate(
      req.params.customerSurgeId,
      {
        ruleName,
        baseFare,
    ...
    updatedCustomerSurge = await updatedCustomerSurge.populate(
      "geofenceId",
      "name"
    );
...
const changeStatusCustomerSurgeController = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      message: "Customer surge status updated successfully",
      data: customerSurgeFound.status,
    });
  } catch (err) {
    next(appError(err.message));
  }
