...
const getMerchantOverview = async (req, res, next) => {
  try {
    ...
    const [overview] = await WhatsappConversation.aggregate([
      {
        $facet: {
          totalOpen: [
            { $match: { status: "open", merchantId } },
            { $count: "count" },
          ],
          totalClosed: [
            { $match: { status: "closed", merchantId } },
            { $count: "count" },
          ],
          totalUnread: [
            { $match: { unreadCount: { $gt: 0 }, merchantId } },
            { $count: "count" },
          ],
          totalConversations: [
            { $match: { merchantId } },
    ...
    const connection = await WhatsappConnection.findOne({ merchantId, status: "Active" }).lean();
    ...
    formatted.connection = connection || null;
...
const getMerchantConversations = async (req, res, next) => {
  try {
    const merchantId = req.merchantId;
    const {
      search = "",
...
const updateMerchantConversation = async (req, res, next) => {
  try {
    const merchantId = req.merchantId;
    ...
    sendSocketData(String(merchantId), "whatsapp:conversation:updated", formatted);
...
const getMerchantMessages = async (req, res, next) => {
  try {
    const merchantId = req.merchantId;
    const { conversationId } = req.params;
...
const sendMerchantMessage = async (req, res, next) => {
  try {
    const merchantId = req.merchantId;
    const { conversationId } = req.params;
    ...
    const metaResponse = await sendMetaMessage(metaPayload, connection);
    ...
    const message = await WhatsappMessage.create({
      ...
      deliveryStatus: "sent",
      senderName: req.userName || "Merchant",
      timestamp: new Date(),
    });
    ...
    sendSocketData(String(merchantId), "whatsapp:message", formatted);
...
const sendMerchantTemplateMessage = async (req, res, next) => {
  try {
    const merchantId = req.merchantId;
    const { conversationId } = req.params;
    ...
    const metaResponse = await sendMetaMessage(payload, connection);
    const metaMessageId = metaResponse.messages?.[0]?.id;
...
const getMerchantNotes = async (req, res, next) => {
  try {
    const merchantId = req.merchantId;
    const { conversationId } = req.params;
    ...
    const notes = await WhatsappNote.find({ conversationId, merchantId })
      .sort({ createdAt: -1 })
      .lean();
...
const addMerchantNote = async (req, res, next) => {
  try {
    const merchantId = req.merchantId;
    const { conversationId } = req.params;
    ...
    const note = await WhatsappNote.create({
      ...
      createdByName: req.userName || "Merchant",
    });

    const formatted = formatNote(note);
    sendSocketData(String(merchantId), "whatsapp:note", formatted);
...
const deleteMerchantNote = async (req, res, next) => {
  try {
    const merchantId = req.merchantId;
    const { conversationId, noteId } = req.params;

    const note = await WhatsappNote.findOneAndDelete({
      _id: noteId,
      conversationId,
      merchantId,
    });
    ...
    sendSocketData(String(merchantId), "whatsapp:note:deleted", { noteId });
